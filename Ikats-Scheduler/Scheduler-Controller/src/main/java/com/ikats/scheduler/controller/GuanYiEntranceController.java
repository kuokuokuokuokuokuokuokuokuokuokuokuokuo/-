        ...
        if (way == 1) {
            ...
            if (method.equals("gy.erp.trade.deliverys.get")) {
                ...
                if (object.containsKey("codes")) {
                    ...
                    {
                        JSONObject thisObject = object;
                        thisObject.remove("codes");
                        thisObject.put("code", codeArray[index]);
                        String sendString = thisObject.toJSONString();
                        String sign = GuanYiPostUtil.sign(sendString, GuanYiPostUtil.SECRET);
                        thisObject.put("sign", sign);
                        String sendParam = thisObject.toJSONString();
                        //管易回执
                        String result = GuanYiPostUtil.sendPost(GuanYiPostUtil.URL, sendParam);
                        ...
                        if (index == codeArray.length - 1) return result;
                    }
                }
                else
                {
