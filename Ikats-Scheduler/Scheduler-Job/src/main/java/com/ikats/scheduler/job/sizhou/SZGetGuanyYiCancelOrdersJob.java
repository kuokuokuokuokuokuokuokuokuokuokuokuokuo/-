...
public class SZGetGuanyYiCancelOrdersJob extends QuartzJobBean {
            ...
            for (ClientBean clientBean : clientBeans)
            {
                ...
                try {
                    ...
                    String clientXml = "<eFreightService>\n" +
                            ...
                            "<ServiceData>\n" +
                            "<ChannelId/>\n" +
                            "<AdminUserName>" + clientBean.getAppKey() + "</AdminUserName>\n" +
                            "<StockId/>\n" +
                            "<ClientId/>\n" +
                            "</ServiceData>\n" +
                            "</eFreightService>\n";
                    ...
                    for (int i = 0; i < client.size(); i++)
                    {
                        for (int page = 1; page < 10000; page++)
                        {
                            ...
                            sendJson.put("appkey", clientBean.getAppKey());
                            sendJson.put("sessionkey", clientBean.getSessionKey());
                            ...
                            String sign = SZGuanYiPostUtil.sign(sendString, clientBean.getAppSecret());
                            sendJson.put("sign", sign);
                            String sendParam = sendJson.toJSONString();
                            //管易回执
                            String result = SZGuanYiPostUtil.sendPost(clientBean.getUrl(), sendParam);
                            ...
                            for (int j = 0; j < deliverys.size(); j++)
                            {
                                ...
                                orderBean.setAppKey(clientBean.getAppKey());
                ...
                }catch (Exception e)
                {
                    ...
                    Email.sendTextMail("liukuo@ikats.com", "四洲取消订单抓取任务", ExceptionUtils.getStackTrace(e));
                    Email.sendTextMail("zhangxiaotao@ikats.com", "四洲取消订单抓取任务", ExceptionUtils.getStackTrace(e));
                    Email.sendTextMail("wuqing@ikats.com", "四洲取消订单抓取任务", ExceptionUtils.getStackTrace(e));
                }
            }
        }
