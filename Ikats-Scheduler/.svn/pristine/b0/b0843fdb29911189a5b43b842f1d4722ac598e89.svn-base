...
@Component
@Transactional
public class GYOrderLogic extends AbstractLogic {

    @Autowired
    private IGYOrderRepository repository;

    /**
     * 添加单条记录
     */
    public void insert(GYOrderBean bean) {
     ...
     * 添加一批记录
     */
    public void insertList(List<GYOrderBean> list)
    {
        //判断订单是否存在
        for (Iterator iter = list.iterator(); iter.hasNext(); )
        {
            GYOrderBean bean = (GYOrderBean) iter.next();
            Map<String,String> express = new HashMap<String, String>();
            express.put("orderNo",bean.getOrderNo());
            express.put("appKey",bean.getAppKey());
            List<GYOrderBean> holdBean = this.repository.selectByNo(express);
            //订单已存在
     ...
     * 更新记录
     */
    public void update(GYOrderBean bean) {
    ...
    public GYOrderBean selectByKey(Long id) {
        GYOrderBean bean = repository.selectByKey(id);
        if (bean == null) {
            this.setSuccess(false);
    ...
    public List<GYOrderBean> selectByQuery(Map<String, String> express) {
        List<GYOrderBean> beanList = repository.selectByQuery(express);
        if (beanList.size() == 0) {
            this.setSuccess(false);
            this.setMessage("没有找到");
            return null;
     ...
     * 得到翻页查询记录
     */
    public List<GYOrderBean> pageByQuery(int pageNum, int pageSize, Map<String, String> express) {
        List<GYOrderBean> beanList = repository.pageByQuery(pageNum, pageSize, express);
        if (beanList.size() == 0) {
            this.setSuccess(false);
            this.setMessage("没有找到");
            return null;
    ...
    {
        List<GYOrderBean> beanList = this.repository.getOrderSendJob();
        if (beanList.size() == 0) {
            this.setSuccess(false);
     ...
     * @return
     */
    public List<GYOrderBean> queryExceptionOrder(int pageNum, int pageSize, Map<String, String> express)
    {
        List<GYOrderBean> beanList = repository.pageByQuery(pageNum, pageSize, express);
        if (beanList.size() == 0)
        {
