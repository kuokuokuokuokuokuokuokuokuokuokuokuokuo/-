...
public class GuanyYiGetOrdersJob extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        try {
            ...
            for (int i = 0; i < client.size(); i++)
            {
                for (int page = 1; page < 10000; page++)
                {
                    ...
                    sendJson.put("end_ modify_date", sdf.format(new Date()));
                    ...
                    for (int j = 0; j < deliverys.size(); j++)
                    {
                        ...
                        String omsXml =
                                "<Service>\n" +
                                        ...
                                        "      <ChannelName>管易</ChannelName>\n" +
                                        "      <Forwarder>" + (StringUtils.isBlank(order.getString("express_no")) ? "CN" + order.get("express_code") : order.get("express_code")) + "</Forwarder>\n" +
                                        "      <SKUDesc></SKUDesc>\n" +
        ...
        } catch (Exception e)
        {
            ...
            Email.sendTextMail("liukuo@ikats.com", "管易订单抓取任务", ExceptionUtils.getStackTrace(e));
            Email.sendTextMail("zhangxiaotao@ikats.com", "管易订单抓取任务", ExceptionUtils.getStackTrace(e));
            Email.sendTextMail("wuqing@ikats.com", "管易订单抓取任务", ExceptionUtils.getStackTrace(e));
        }
