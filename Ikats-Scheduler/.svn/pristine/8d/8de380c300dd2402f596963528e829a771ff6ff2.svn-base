...
import com.ikats.scheduler.logic.ClientLogic;
...
public class SZGetGuanYiOrdersJob extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        if (null == orderLogic || null == clientLogic)
        {
            SystemOutMessage.body("false 系统异常");
            return;
        }
        List<ClientBean> clientBeans = this.clientLogic.selectEffectiveClient();
        if(null != clientBeans && clientBeans.size()>0)
        {
            for(ClientBean clientBean : clientBeans)
            {
                ...
                try {
                    ...
                    String clientXml = "<eFreightService>\n" +
                            ...
                            "<ServiceData>\n" +
                            "<ChannelId/>\n" +
                            "<AdminUserName>" + clientBean.getAppKey().trim() + "</AdminUserName>\n" +
                            "<StockId/>\n" +
                            "<ClientId/>\n" +
                            "</ServiceData>\n" +
                            "</eFreightService>\n";
                    ...
                    if (null == jsonClients)
                    {
                        SystemOutMessage.body("OMS --  client 查询失败 -- 终止订单抓取");
                        return;
                    }
                    ...
                    for (int i = 0; i < client.size(); i++)
                    {
                        for (int page = 1; page < 10000; page++)
                        {
                            ...
                            sendJson.put("appkey", clientBean.getAppKey().trim());//SZGuanYiPostUtil.APPKEY
                            sendJson.put("sessionkey", clientBean.getSessionKey().trim());//SZGuanYiPostUtil.SESSIONKEY
                            ...
                            sendJson.put("start_modify_date", sdf.format(JobDateUtil.backToThePastMinute(now, 20).getTimeInMillis()));
                            sendJson.put("page_no", page);
                            sendJson.put("page_size", "100");
                            sendJson.put("delivery", "0");
                            String sendString = sendJson.toJSONString();
                            String sign = SZGuanYiPostUtil.sign(sendString,clientBean.getAppSecret().trim());//SZGuanYiPostUtil.SECRET
                            sendJson.put("sign", sign);
                            String sendParam = sendJson.toJSONString();
                            //管易回执
                            String result = SZGuanYiPostUtil.sendPost(clientBean.getUrl().trim(), sendParam);//SZGuanYiPostUtil.URL
                            ...
                            for (int j = 0; j < deliverys.size(); j++)
                            {
                                ...
                                orderBean.setAppKey(clientBean.getAppKey().trim());//SZGuanYiPostUtil.APPKEY
                ...
                } catch (Exception e)
                {
                    ...
                    Email.sendTextMail("liukuo@ikats.com", "四洲订单抓取任务", ExceptionUtils.getStackTrace(e));
                    Email.sendTextMail("zhangxiaotao@ikats.com", "四洲订单抓取任务", ExceptionUtils.getStackTrace(e));
                    Email.sendTextMail("wuqing@ikats.com", "四洲订单抓取任务", ExceptionUtils.getStackTrace(e));
                }
            }
        }
