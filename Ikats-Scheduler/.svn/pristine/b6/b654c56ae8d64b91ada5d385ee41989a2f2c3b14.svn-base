            ...
            for (ClientBean clientBean : clientBeans)
            {
                ...
                try {
                    for (int page = 1; page < 10000; page++)
                    {
                        ...
                        sendJson.put("appkey", clientBean.getAppKey().trim());
...
//                sendJson.put("code","LFQD0073-1");
                        ...
                        for (int i = 0; i < items.size(); i++)
                        {
                            ...
                            if (combine.equals("true"))//组合商品
                            {
                                ...
                                skuBean.setAppKey(clientBean.getAppKey().trim());//SZGuanYiPostUtil.APPKEY
                                ...
                                if(skus.size() == 0)//单一商品没有sku
                                {
                                    ...
                                    barJson.put("appkey",clientBean.getAppKey().trim());
                                    barJson.put("sessionkey",clientBean.getSessionKey().trim());
                                    ...
                                    String barSign = SZGuanYiPostUtil.sign(barString, clientBean.getAppSecret().trim());
                                    barJson.put("sign", barSign);
                                    String barParam = barJson.toJSONString();
                                    //管易回执
                                    String abrResult = SZGuanYiPostUtil.sendPost(clientBean.getUrl().trim(), barParam);
                                    ...
                                    GYSkuBean skuBean = new GYSkuBean();
                                    skuBean.setIdRecord(1L);
                                    skuBean.setAppKey(clientBean.getAppKey().trim());
                                    skuBean.setState(SendStatus.INIT.getValue());
                                    skuBean.setTimes(0);
                                ...
                                }else//有sku
                                {
                                    for (int j = 0; j < skus.size(); j++)
                                    {
                                        ...
                                        GYSkuBean skuBean = new GYSkuBean();
                                        skuBean.setIdRecord(1L);
                                        skuBean.setAppKey(clientBean.getAppKey());
                                        skuBean.setState(SendStatus.INIT.getValue());
                                        skuBean.setTimes(0);
                                        skuBean.setCombine("0");
